...
#import "GSKeychainStore.h"

static NSString *defaultService;

@interface GSKeychainStore () {
    NSMutableDictionary *itemsToUpdate; //将要提交到Keychain的条目
}

@end

@implementation GSKeychainStore
...
+ (BOOL)removeItemForKey:(NSString *)key {
    return [GSKeychainStore removeItemForKey:key service:defaultService accessGroup:nil];
}

+ (BOOL)removeItemForKey:(NSString *)key service:(NSString *)service {
    return [GSKeychainStore removeItemForKey:key service:service accessGroup:nil];
}
...
+ (BOOL)removeAllItemsForService:(NSString *)service accessGroup:(NSString *)accessGroup {
    NSArray *items = [GSKeychainStore itemsForService:service accessGroup:accessGroup];
...
- (NSString *)description {
    NSArray *items = [GSKeychainStore itemsForService:self.service accessGroup:self.accessGroup];
